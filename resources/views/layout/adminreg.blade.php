...
<div class="container mt-4">
    <div class="card p-4 shadow">
        ...
        <form method="POST" action="{{isset($profile) ? route('adminUserAdded',$profile->id) : route('adminUserAdded')}}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="col-md-6 mb-3">
                ...
                <select  name="profile_for" class="form-control">
                   ...
                   <option value="groom"{{ old('profile_for', $profile->user->profile_for ) == 'groom' ? 'selected' : '' }}Groom</option>
                    <option value="bride"{{ old('profile_for', $profile->user->profile_for ) == 'bride' ? 'selected' : '' }}Bride</option>
                </select>
            </div>
                ...
                <div class="col-md-6 mb-3">
                    <label>Email</label>
                    <input type="email" name="email" class="form-control" value="{{old('name',$profile->user->email)}}">
                </div>
...
@enderror
                ...
                <div class="col-md-6 mb-3">
                    <label>Phone</label>
                    <input type="text" name="phone" class="form-control" value="{{old('name',$profile->user->phone)}}">
                </div>

                  <div class="col-md-6 mb-3">
                    <label>Height</label>
                    <input type="text" name="height" class="form-control" value="{{old('name',$profile->height)}}">
                </div>
    ...
    <div class="col-md-4">
        ...
        <select name="year" class="form-control"  >
            ...
            @endfor
                ...
                <div class="col-md-6 mb-3">
                    <label>Age</label>
                    <input type="text" name="age" class="form-control" value="{{old('name',$profile->age)}}">
                </div>
                <div class="col-md-6 mb-3">
                ...
                <div class="col-md-6 mb-3">
                 ...
                 <div class="col-md-6 mb-3">
                    ...
                    <select name="" id="" name="status "class="form-control"  >
                    <option value="" >Choose Status</option> 
                       
                   <option value="never married"{{ old('status', $profile->status ) == 'never married' ? 'selected' : '' }}Never Married</option>
                        <option value="married"{{ old('status', $profile->status ) == 'married' ? 'selected' : '' }}Married</option>
                        <option value="divorced"{{ old('status', $profile->status ) == 'divorced' ? 'selected' : '' }}Divorced</option>
                        <option value="widowed"{{ old('status', $profile->status ) == 'widowed' ? 'selected' : '' }}Widowed</option>
                     ...
                     @for($i = 1; $i <= 4; $i++)
                        <div class="upload-box">
                         <input type="file" name="image{{ $i }}" accept="image/*" onchange="previewImage(event, {{ $i }})">
                             ...
                             <img id="preview{{ $i }}"   src="{{ isset($profile) && $profile->{'image'.$i} ? asset('images/'.$profile->{'image'.$i}) : '' }}">
